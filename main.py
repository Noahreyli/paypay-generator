...
os.system("cls")
os.system("title PayPay Generator x Checker")
...
{Color.RESET}----------- 作者:{Color.RED}rucykun{Color.RESET} ----------
-------------------- Version:{Color.RED}0.0.1{Color.RESET} --------------------
    """)
    print("""
[1] PayPay Generator
[2] PayPay Checker
    ...
    if number == "1":
        ...
        with open(f"paylink.txt", "w", encoding="utf-8") as f:
            for i_ in li:
                f.write(f"{i_}\n")
        time.sleep(1)
        print(f"{str(amount)} 個のリンクを生成しました。")
        ...
        if os.path.isfile(f"{files}") is True:
            delay = input("チェック間隔を設定してください(推奨:3以上):")
...
{Color.RED}[FAILURE] 合計 : {str(failure)} 個{Color.RESET}
            ...
            print(f"{Color.RED}[エラー]指定したファイル({files})が見つかりませんでした。{Color.RESET}")
            time.sleep(1)
            input("なんらかのキーを押すと終了します")
            main()
